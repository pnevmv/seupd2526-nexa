...
To verify that the MRR@5 differences reported above are not due to sampling variance, we performed a two-way ANOVA on the per-query RR@5 values, blocking on topics~\citep{Sakai2016}. The two-way design accounts for query-level difficulty, which increases statistical power compared to a one-way test. The three configurations (Lexical, Hybrid, Cross-Encoder) act as treatment factors; each query constitutes a block that is evaluated by all three systems simultaneously. Figure~\ref{fig:rr5-dev} shows the distribution of the first relevant rank for each language and Figure~\ref{fig:rr5-tukey} reports the two-way ANOVA $F$-statistics and the post-hoc Tukey HSD pairwise comparisons ($\alpha = 0.05$).

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rank_distribution_dev.pdf}
  \caption{Distributions of the rank of the first relevant document for the three system configurations on the dev dataset.}
  \label{fig:rr5-dev}
\end{figure}
...
We repeated the two-way ANOVA and Tukey HSD analysis on the test set, replacing the base cross-encoder with the fine-tuned model described in Section~\ref{subsubsec:res-ce}. The three configurations compared are the lexical baseline (Run~1), the BGE-M3 hybrid ($\alpha{=}0.80$, Run~2), and the fine-tuned cross-encoder reranking the top-20 hybrid candidates. Figure~\ref{fig:rr5-test} shows the first relevant rank distributions by language and Figure~\ref{fig:rr5-tukey-test} the Tukey HSD heat-maps.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rank_distribution_test.pdf}
  \caption{First relevent rank distribution on the test set.}
  \label{fig:rr5-test}
\end{figure}
