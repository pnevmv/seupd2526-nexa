...
We now tune the query construction process by adjusting field boosts, deciding whether terms are mandatory or optional, and controlling the level of query pruning. Table~\ref{tab:field-boosts} sweeps the title, abstract, and authors boosts in three parts (title with abstract fixed, abstract with title fixed, then authors on top of the winning ratio). Table~\ref{tab:topk-pruning} then prunes the query to the top-$K$ tokens by IDF\@. The strictness of the Boolean query (OR vs.\ AND) is reported in prose, since the effect is too one-sided to need a table.
...
The principal challenge in this task is a vocabulary gap between informal, tweet-style claims and formal scientific abstracts. Section~\ref{sec:methodology} describes four expansion strategies including a static synonym dictionary, an LLM-based expander (Groq API), WordNet synonym injection, and pseudo-relevance feedback (PRF). The LLM-based and static dictionary strategies were not included in the systematic sweep reported here since the Groq API is rate-limited to approximately 30 requests per minute, making batch evaluation over thousands of claims impractical, and initial experiments with the static dictionary showed no measurable gain over the unexpanded baseline. For this reason, we evaluate WordNet and PRF, as they are self-contained and reproducible. We tested both on top of the analyzer-fixed configuration from Section~\ref{subsec:res-analyzer}, before the fielded boosting described in Section~\ref{subsec:res-fielded}; this staging keeps the expansion experiments independent of the fielded tuning. In both cases, retrieval performance decreased as expansion grows.
...
The two queryMode rows reproduce the collapse already seen in Section~\ref{subsec:res-fielded}: switching just the two highest-IDF terms to AND drops the average to 0.087, and a fully AND query is essentially zero in every language. No claim text appears verbatim in any abstract, so the system depends on every term being optional. This confirms OR as the only viable Boolean mode and closes the lexical tuning.
...
Combining the winners of each phase gives the two configurations we submitted to the official evaluation. Run~1 is the pure lexical system, included as a reproducible baseline that depends only on the Lucene index and requires no GPU at query time. Run~2 is the primary submission: the same lexical system with BGE-M3 score fusion at $\alpha{=}0.70$, which was the best configuration identified on the dev set.
...
Run~2 gains 5.7 points of average MRR@5 over Run~1 on the dev set. The improvement is consistent across all three languages: $+6.0$ points on EN, $+6.2$ on FR, and $+4.9$ on DE, so no single language is disproportionately favoured or penalised by the fusion. All hyperparameters (field boosts, $k_1$, $b$, $\alpha$, and topK) were tuned only on the dev split.

Table~\ref{tab:test-results} reports the official test-set scores for Run~2.

\begin{table}[!ht]
    \caption{Official test-set MRR@5 for the primary submitted run (Run~2: hybrid lexical $+$ BGE-M3 fusion, $\alpha{=}0.70$).}
...
The test-set average of 0.5435 is 1.5 points above the dev-set result of 0.5290, indicating that the system did not overfit to the dev split. Per-language results are mixed: EN and FR drop slightly from dev to test ($-0.7$ and $-0.3$ points respectively), while DE improves by a substantial 5.4 points. The DE gain is consistent with the dev set being a harder sample for German. Its dev-set scores were lower throughout the tuning phase, so the test distribution appears somewhat more favourable for DE\@. Overall, the hybrid system transfers reliably across the unseen data, and the dense fusion gain over a pure lexical run is confirmed on unseen data.

\subsubsection*{Dev-set breakdown analysis}

To see where the MRR@5 gains come from, Table~\ref{tab:dev-breakdown} breaks the dev-set scores into hit rates and the fraction of queries for which no relevant document appeared in the top 100 (missing). All three system configurations are shown for comparison, including the fine-tuned cross-encoder from Section~\ref{subsubsec:res-ce} (even though it was not submitted).

\begin{table}[!ht]
    \caption{Dev-set retrieval breakdown for all three system configurations. Hit@1 and Hit@5 are cumulative (percentage of queries with the correct document ranked $\leq\!1$ and $\leq\!5$, respectively); Missing is the fraction of queries where the correct document does not appear in the top 100.}
    \label{tab:dev-breakdown}
    \centering
    \begin{tabular}{llcccc}
        \toprule
        ...
        \midrule
        Hybrid ($\alpha{=}0.70$)    & EN & 0.5875 & 53.0 & 67.7 & 14.6 \\
...
\paragraph{Hybrid gains come from recall, not re-ranking.}
The lexical system misses any relevant document in the top 100 for about one in four queries on EN and FR, and one in three on DE\@. The hybrid cuts these missing rates to 14--19\%, while also improving Hit@1 by 6--8 percentage points per language. Together these two effects explain the $+7$ to $+8.5$\,pt MRR@5 gain: the dense signal finds documents BM25 misses entirely, and ranks them higher when BM25 does find them.

\paragraph{Cross-encoder: precision up, recall down, asymmetric across languages.}
The CE only reranks the top-50 hybrid candidates, so any document at hybrid ranks 51--100 is effectively discarded. This raises the missing rate by $+8.6$ pp for EN, $+8.8$ pp for FR, and $+17.6$ pp for DE\@. Despite this recall hit, the CE still comes out ahead on FR ($+2.2$ pts MRR@5) and DE ($+1.7$ pts), because it promotes more queries to rank 1 in those languages ($+3.0$ pp and $+2.1$ pp Hit@1). For EN it is the opposite: the CE pushes 138 queries down from rank~1, costing $-2.7$\,pt MRR@5, which is the primary reason we did not submit the fine-tuned reranker.

\paragraph{German is the hardest language.}
DE trails EN and FR by 13--14 MRR@5 points in every configuration. Its lexical missing rate (35.2\%) is about ten percentage points worse than EN and FR, and the CE pushes it back almost to that level (37.0\%), suggesting that the hybrid's recall gain on DE is fragile once the candidate list is cut to 50. The hybrid is the configuration that helps DE the most in absolute terms ($-15.8$ pp missing), which fits with multilingual BGE-M3 doing a better job at cross-lingual matching than the lexical index can achieve alone.

\subsection{Statistical Analysis}
\label{subsec:res-stats}

To verify that the MRR@5 differences reported above are not due to sampling variance, we performed a two-way ANOVA on the per-query RR@5 values, blocking on topics~\citep{Sakai2016}. The two-way design accounts for query-level difficulty, which increases statistical power compared to a one-way test. The three configurations (Lexical, Hybrid, Cross-Encoder) act as treatment factors; each query constitutes a block that is evaluated by all three systems simultaneously. Figure~\ref{fig:rr5-boxplot} shows the resulting RR@5 distributions per language, and Figure~\ref{fig:rr5-tukey} reports the two-way ANOVA $F$-statistics and the post-hoc Tukey HSD pairwise comparisons ($\alpha = 0.05$).

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rr5_boxplot.pdf}
  \caption{Per-query RR@5 distributions for the three system configurations across all dev queries. Diamonds mark the per-system mean. The median line and whiskers follow the standard Tukey convention (1.5$\times$IQR).}
  \label{fig:rr5-boxplot}
\end{figure}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rr5_tukey.pdf}
  \caption{Two-way ANOVA results and Tukey HSD post-hoc pairwise comparisons per language. Each off-diagonal cell shows the mean RR@5 difference (row\,$-$\,col) and the significance marker. Cell colour encodes the Tukey $p$-value: green indicates a significant difference ($p < 0.05$), red a non-significant one.}
  \label{fig:rr5-tukey}
\end{figure}

\begin{table}[!ht]
  \caption{Two-way ANOVA results per language. All three languages reject $H_0$ at $\alpha = 0.05$.}
  \label{tab:anova}
  \centering
  \begin{tabular}{lrrrrl}
    \toprule
    Language & $n$ queries & $F$ & $p$-value & $\alpha$ & Decision \\
    \midrule
...
\subsection{Test-Set Statistical Analysis}
\label{subsec:res-stats-test}

We repeated the two-way ANOVA and Tukey HSD analysis on the test set, replacing the base cross-encoder with the fine-tuned model described in Section~\ref{subsubsec:res-ce}. The three configurations compared are the lexical baseline (Run~1), the BGE-M3 hybrid ($\alpha{=}0.70$, Run~2), and the fine-tuned cross-encoder reranking the top-20 hybrid candidates. Figure~\ref{fig:rr5-boxplot-test} shows the per-query RR@5 distributions and Figure~\ref{fig:rr5-tukey-test} the Tukey HSD heat-maps.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rr5_boxplot_test.pdf}
  \caption{Per-query RR@5 distributions on the test set. Diamonds mark the per-system mean; whiskers follow the Tukey 1.5$\times$IQR convention.}
  \label{fig:rr5-boxplot-test}
\end{figure}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rr5_tukey_test.pdf}
  \caption{Two-way ANOVA and Tukey HSD post-hoc comparisons on the test set (fine-tuned CE). Cell colour and annotation conventions are the same as Figure~\ref{fig:rr5-tukey}.}
  \label{fig:rr5-tukey-test}
\end{figure}

\begin{table}[!ht]
  \caption{Two-way ANOVA results on the test set. All three languages reject $H_0$ at $\alpha = 0.05$.}
...
Table~\ref{tab:anova-test} confirms that at least one system differs significantly on every language partition. The Tukey HSD post-hoc results are:

\begin{itemize}
  \item \textbf{Lexical vs.\ Hybrid} is significant in all three languages ($p < 0.001$), replicating the dev-set finding on held-out data with substantially larger query sets.
  \item \textbf{Lexical vs.\ Cross-Encoder} is significant in all three languages ($p < 0.001$ for EN and FR; $p = 0.000005$ for DE), confirming that the fine-tuned CE consistently outperforms the pure lexical system.
  \item \textbf{Hybrid vs.\ Cross-Encoder} is significant in English ($p < 0.001$, Hybrid $>$ CE by $\Delta{=}0.030$) and German ($p = 0.035$, Hybrid $>$ CE by $\Delta{=}0.030$). In French the difference is not significant ($p = 0.054$, Hybrid $>$ CE by $\Delta{=}0.023$), indicating that fine-tuned CE and hybrid are statistically equivalent on that language.
\end{itemize}
...
\begin{table*}[t]
\centering
\caption{Detailed variance decomposition (SS, df, MS) for the Two-way ANOVA on Development and Test sets}
\label{tab:anova_variance_details}
\small
...
\hline
\textbf{Dataset} & \textbf{Language} & \textbf{Source of Variation} & \textbf{SS} & \textbf{df} & \textbf{MS} \\
\hline
\hline
\textbf{Development Set} 
& English & Systems & 20.6108 & 2 & 10.3054 \\
& & Topics (Queries) & 2065.5555 & 3904 & 0.5291 \\
& & Error & 429.5688 & 7808 & 0.0550 \\
\cline{2-6}
...
\hline
\textbf{Test Set} 
& English & Systems & 12.6469 & 2 & 6.3234 \\
...
\begin{flushleft}
\normalsize
\textbf{Key Statistical Metrics Defined:}
\begin{itemize}
    \item \textbf{SS (Sum of Squares):} Measures the total variability or dispersion of the data relative to the mean. It quantifies the total deviation associated with a specific source of variation (e.g., the effect of the systems, the effect of the queries, or pure random error).
    
    \item \textbf{df (Degrees of Freedom):} Represents the number of values that are "free" to vary within the data, or the amount of independent information available to estimate variability. 
    
    \item \textbf{MS (Mean Square):} Calculated by dividing the Sum of Squares by the respective Degrees of Freedom ($MS = \frac{SS}{df}$). It represents the standardized estimate of variance for each component.
\end{itemize}

\vspace{0.2cm}
\noindent \textbf{Why are they useful?} \\
In a Two-way ANOVA, these three elements work together to decompose the total variability. Isolating the values of \textit{SS}, \textit{df}, and \textit{MS} makes it possible to understand how much of the variation in the results is attributable to the factors of interest (the \textit{Systems} or \textit{Topics}) and how much is simply "noise" (\textit{Error}). Most importantly, the ratio between the MS of a given factor and the MS of the error generates the $F$-statistic, which is the key value used to determine whether the observed differences in performance are statistically significant or simply due to chance.
\end{flushleft}
