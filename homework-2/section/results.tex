...
We repeated the two-way ANOVA and Tukey HSD analysis on the test set with the same three configurations as on dev: the lexical baseline (Run~1), the BGE-M3 hybrid ($\alpha{=}0.80$, Run~2), and the fine-tuned cross-encoder of Section~\ref{subsubsec:res-ce} applied on top of Run~2. Figure~\ref{fig:rr5-test} shows the first relevant rank distributions by language and Figure~\ref{fig:rr5-tukey-test} the Tukey HSD heat-maps.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{figure/rank_distribution_test.pdf}
  \caption{Rank of the relevant document per query, test set. Bucket conventions follow Figure~\ref{fig:rr5-dev}.}
  \label{fig:rr5-test}
\end{figure}
...
\begin{tabular}{lllccc}
    ...
    & \multirow{3}{*}{English} & Systems          &    9.8397 &  2    &  4.9198 \\
