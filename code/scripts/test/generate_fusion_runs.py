#!/usr/bin/env python3
"""Apply BM25+embedding score fusion at a fixed alpha and write TREC run files.

Reads lexical run files + precomputed doc embeddings, encodes queries with
BGE-M3, applies (1-alpha)*BM25norm + alpha*cosine, and writes TREC output.

Usage:
    python3 generate_fusion_runs.py [--alpha 0.70] [--top-k 50] [--batch-size 32]
"""
...
LANG_CONFIGS = {
    "en": {
        ...
        "run_file":   REPO_ROOT / "runs/abl-Full_best_reference-en.txt",
        "out_file":   REPO_ROOT / "runs/hybrid-en-dev-bge-fusion.txt",
        "run_tag":    "hybrid-en-dev-bge-fusion",
    },
    "fr": {
        "query_file": REPO_ROOT / "datasets/processed/queries/translated/fr_dev_en_translated.json",
        "run_file":   REPO_ROOT / "runs/abl-Full_best_reference-fr.txt",
        "out_file":   REPO_ROOT / "runs/hybrid-fr-dev-bge-fusion.txt",
        "run_tag":    "hybrid-fr-dev-bge-fusion",
    },
    "de": {
        "query_file": REPO_ROOT / "datasets/processed/queries/translated/de_dev_en_translated.json",
        "run_file":   REPO_ROOT / "runs/abl-Full_best_reference-de.txt",
        "out_file":   REPO_ROOT / "runs/hybrid-de-dev-bge-fusion.txt",
        "run_tag":    "hybrid-de-dev-bge-fusion",
    },
}
