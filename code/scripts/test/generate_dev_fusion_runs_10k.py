#!/usr/bin/env python3
"""Full 10k BM25+BGE-M3 fusion dev run files at a given alpha.

Reads wider-*-dev-lexical.txt (top-1000 BM25), computes BGE-M3 cosine over
all 10k docs, fuses at alpha=0.80, and writes TREC run files.

Usage:
    python3 generate_dev_fusion_runs_10k.py [--alpha 0.80] [--top-k 100] [--batch-size 32]
"""
...
LANG_CONFIGS = {
    "en": {
        ...
        "out_file":   REPO_ROOT / "runs/hybrid-en-dev-bge-fusion-10k.txt",
        "run_tag":    "hybrid-en-dev-bge-fusion-10k",
    },
    "fr": {
        ...
        "out_file":   REPO_ROOT / "runs/hybrid-fr-dev-bge-fusion-10k.txt",
        "run_tag":    "hybrid-fr-dev-bge-fusion-10k",
    },
    "de": {
        ...
        "out_file":   REPO_ROOT / "runs/hybrid-de-dev-bge-fusion-10k.txt",
        "run_tag":    "hybrid-de-dev-bge-fusion-10k",
    },
}
...
def main():
    ...
    parser.add_argument("--top-k",      type=int,   default=None,
                        help="Max results per query (default: all 10k)")
    parser.add_argument("--batch-size", type=int,   default=32)
    args = parser.parse_args()

    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"Device: {device}  alpha={args.alpha}  top-k={args.top_k or 'all'}")
    ...
    for lang, cfg in LANG_CONFIGS.items():
        ...
        top_k    = args.top_k or len(all_pks)
        suffix   = f"-top{args.top_k}" if args.top_k else ""
        out_path = cfg["out_file"].with_name(cfg["out_file"].stem + suffix + cfg["out_file"].suffix)
        tag      = cfg["run_tag"] + (f"-top{args.top_k}" if args.top_k else "")
        written  = 0
