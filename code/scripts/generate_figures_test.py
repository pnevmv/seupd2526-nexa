"""
Generate publication-quality figures for the test-set statistical analysis section.

Produces:
  homework-2/figure/rr5_boxplot_test.pdf   – per-query RR@5 box plots, 3 languages
  homework-2/figure/rr5_tukey_test.pdf     – Tukey HSD significance heat-map
"""
...
plt.savefig(f"{OUT}/rr5_boxplot_test.pdf", bbox_inches="tight", dpi=150)
plt.close()
print("Saved rr5_boxplot_test.pdf")


# ---------------------------------------------------------------------------
# FIGURE 2 — Tukey HSD heat-map
# ---------------------------------------------------------------------------
...
for ax, lang, ll in zip(axes, langs, lang_lbl):
    ...
    anova_p = res["p"]
    sig_str = "$p < 0.001$" if anova_p < 0.001 else f"$p = {anova_p:.3f}$"
...
plt.savefig(f"{OUT}/rr5_tukey_test.pdf", bbox_inches="tight", dpi=150)
plt.close()
print("Saved rr5_tukey_test.pdf")
