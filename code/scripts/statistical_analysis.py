...
RUN_FILES = {
    "en": {
        "Lexical":       str(REPO_ROOT / "runs/wider-en-dev-lexical.txt"),
        "Hybrid":        str(REPO_ROOT / "runs/hybrid-en-dev-bge-fusion-10k-top100.txt"),
        "CrossEncoder":  str(REPO_ROOT / "runs/en_dev_ce_reranked.txt"),
    },
    "fr": {
        "Lexical":       str(REPO_ROOT / "runs/wider-fr-dev-lexical.txt"),
        "Hybrid":        str(REPO_ROOT / "runs/hybrid-fr-dev-bge-fusion-10k-top100.txt"),
        "CrossEncoder":  str(REPO_ROOT / "runs/fr_dev_ce_reranked.txt"),
    },
    "de": {
        "Lexical":       str(REPO_ROOT / "runs/wider-de-dev-lexical.txt"),
        "Hybrid":        str(REPO_ROOT / "runs/hybrid-de-dev-bge-fusion-10k-top100.txt"),
        "CrossEncoder":  str(REPO_ROOT / "runs/de_dev_ce_reranked.txt"),
    },
}

DEV_FILES = {
    "en": str(REPO_ROOT / "datasets/en_dev.json"),
    "fr": str(REPO_ROOT / "datasets/fr_dev.json"),
    "de": str(REPO_ROOT / "datasets/de_dev.json"),
}
...
if __name__ == "__main__":
    ...
    for lang in ["en", "fr", "de"]:
        ...
        if dev_exists and runs_exist:
            ...
            missing_parts = []
            if not dev_exists:
                missing_parts.append(f"missing {os.path.basename(DEV_FILES[lang])}")
            if not runs_exist:
    ...
    if len(active_langs) > 0:
        ...
        print("\nNo languages have complete datasets and runs to perform pooled statistical analysis.")
