...
On top of the BGE-M3 fusion run we applied the fine-tuned cross-encoder described in Section~\ref{sec:methodology}. Table~\ref{tab:ce-finetune} compares the base checkpoint against the task-specific fine-tuned model, both reranking the top-50 candidates of the hybrid run.

\begin{table}[!ht]
    \caption{Cross-encoder reranking on top of the BGE-M3 hybrid run, MRR@5. Base is \texttt{ms-marco-MiniLM-L-6-v2}; Fine-tuned is the same model after 2-epoch training on hard negatives from our retrieval pipeline.}
    ...
    \begin{tabular}{lcccc}
        ...
        Method & EN & FR & DE & Avg \\
        \midrule
        Hybrid baseline (BGE-M3 fusion) & 0.5875 & 0.5990 & 0.4615 & 0.5493 \\
        $+$ CE reranking (base)         & 0.5875 & 0.5990 & 0.4615 & 0.5493 \\
