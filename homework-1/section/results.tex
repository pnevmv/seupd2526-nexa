...
Parts A and B fix the title-to-abstract boost ratio. With the abstract held at 1.0, raising the title boost above 1.0 only hurts; with the title held at 2.0, raising the abstract boost up to 3.0 monotonically improves. The winning combination is title$^2$ abstract$^3$, and what matters is the 2:3 ratio rather than the absolute values. Claims are tweet-style paraphrases of findings, not of titles, so the abstract is the primary evidence field, while the title still contributes a smaller but non-zero signal.

Part C adds the authors field on top of title$^2$ abstract$^3$, with the peak at authors$^{1.5}$ (0.4732 average), about 1.5 points above the no-authors row. Pushing higher hurts as mismatched proper nouns start to dominate. The DE column gains the most (about 2.4 points), consistent with German-translated tweets more often naming researchers than the English originals. We also swept a venue boost from 0.0 to 0.5 with no measurable effect.
